...
describe("LocalMultiSig contract", function () {
    ...
    beforeEach(async function () {
      ...
      result = await multisig_contract.submit(addr3.address, ethers.utils.parseEther("0.1"))
      result = await multisig_contract.submit(addr3.address, ethers.utils.parseEther("0.01"))
    ...
    describe("Transaction Submittion", function () {
        it("Owners Can submit transactions", async function(){
            result = await multisig_contract.connect(addr1).submit(addr3.address, ethers.utils.parseEther("0.001"))
        });
        ...
        it("Fail if msg sender is not owner", async function(){
            ...
            await expect(
                multisig_contract.connect(addr3).submit(addr3.address, ethers.utils.parseEther("0.001"))
