...
contract MultiSig{
    ...
    // Defining events
    // Fired when tokens are deposited to this multisig wallet without function name
    // The fallback 'receive' function is called
    event FallbackDeposit(address indexed sender, uint amount);
    // Fired when tokens are deposited to this multisig wallet via payment function
    // Used for the nft sales, accesing any functional aspects of the system
    // Data will carry the information for the transacrtion
    // event ContractDeposit(address indexed sender, uint amount, bytes data);
    ...
    (bool os, ) = payable(owners[0]).call{value: address(this).balance}("");
    require(os);
    }
}