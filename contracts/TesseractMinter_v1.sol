...
contract TesseractMinter_v1 {
    
    // Cost of Minting one NFT
    uint256 public mintCost = 1200 * (10**18); //1000 IoTX/Eth
    ...
    mapping(address => uint256) public TesseractCount;
    ...
    * Mints some NFTs
    */
    function mintTesseract(uint numberOfTokens) public payable pausable {
        // require(saleIsActive, "Sale must be active to mint Tesseract");
        require(numberOfTokens <= maxUserMints, "Can only mint 20 tokens at a time");
        require(TesseractCount[msg.sender] + numberOfTokens <= 20, "Exceeds User Tesseract Limit");
